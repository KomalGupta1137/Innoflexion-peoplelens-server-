select distinct RangeNo as "rangeNo",
    min(attainment) over(partition by RangeNo) as "minRangeVal",
    max(attainment) over(partition by RangeNo) as "maxRangeVal",
    avg(attainment) over(partition by RangeNo) as "avgRangeVal",
    count(*) over(partition by RangeNo) as "rangeCount",
    min(attainment) over() as "minMetricVal",
    max(attainment) over() as "maxMetricVal",
    avg(attainment) over() as "avgMetricVal"
from (
        SELECT round(
