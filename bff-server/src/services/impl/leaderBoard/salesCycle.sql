select
    distinct RangeNo as "rangeNo",
    min(totSalesCycle) over(partition by RangeNo) as "minRangeVal",
    max(totSalesCycle) over(partition by RangeNo) as "maxRangeVal",
    avg(totSalesCycle) over(partition by RangeNo) as "avgRangeVal",
    count(*) over(partition by RangeNo) as "rangeCount",
    min(totSalesCycle) over() as "minMetricVal",
    max(totSalesCycle) over() as "maxMetricVal",
    avg(totSalesCycle) over() as "avgMetricVal"
from
(
        select
            round(
